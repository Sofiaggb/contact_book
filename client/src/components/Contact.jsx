...
export const Contact = ({idContact, onEdit, onBack, onDelete} ) => {
  ...
  useEffect(() => {
    const fetchContact = async () => {
      ...
      try {
        const data = await getContactById(idContact);
        setContact(data.contact); // Asumiendo que la respuesta tiene una propiedad 'contact'
      } catch (error) {
  ...
  if (error) {
    return <div>Error al cargar el contacto.</div>; // Muestra un mensaje de error
  }

  if (!contact) {
    return <div>Contacto no encontrado.</div>; // Maneja el caso de contacto no encontrado
  }
  return (
